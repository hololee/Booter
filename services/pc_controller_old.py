...
class PCController:
    ...
    async def get_pc_status(self, pc_id: str) -> dict:
        ...
        try:
            ...
            return {
                ...
                "active_tasks": len([t for t in self.boot_tasks.values() if not t.is_completed])
            }
    ...
    async def boot_ubuntu(self) -> str:
        """우분투 부팅"""
        task_id = str(uuid.uuid4())
        task = BootTask("Ubuntu", task_id)
    ...
    async def _boot_ubuntu_process(self, task: BootTask):
        ...
        try:
            ...
            boot_success = await self._wait_for_ubuntu_boot(task)
    ...
    async def boot_windows(self) -> str:
        """윈도우 부팅"""
        task_id = str(uuid.uuid4())
        task = BootTask("Windows", task_id)
        self.boot_tasks[task_id] = task
    ...
    async def _boot_windows_process(self, task: BootTask):
        ...
        try:
            ...
            windows_success = await self._wait_for_windows_boot(task)
    ...
    async def _wait_for_ubuntu_boot(self, task: BootTask) -> bool:
        """우분투 부팅 대기"""
    ...
    async def _wait_for_windows_boot(self, task: BootTask) -> bool:
        """윈도우 부팅 대기"""
        timeout = self.max_boot_time
        start_time = asyncio.get_event_loop().time()
